...
try {
   ...
   $date = Get-Date
   $terminateDate = $date.AddDays(14)
   $expireString  = Get-Date -format 'MM/dd/yyyy' -Date $terminateDate
...
#example-call syntax
.\Create-TwoWeekLimitedLabVirtualMachine.ps1 -ResourceGroupName '<lab resource group name>' -LabName '<lab name>' -userName '<VM administrative username>' -password '<VM admin password>' -VMName '<VM name to create>'